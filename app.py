...
def get_client():
    try:
        # 1. 키 데이터 (복사 시 오차를 최소화하기 위해 공백 없이 조립합니다)
        k_parts = [
        ...
        # 2. 모든 줄바꿈과 공백을 제거한 후 정석 PEM 규격으로 강제 재조립
        clean_key = "".join(k_parts).replace(" ", "").replace("\n", "")
    ...
    if uploaded_file:
        with st.spinner('AI 분석 중...'):
            try:
                ...
                st.success("✅ 분석 완료!")
                st.text_area("결과 확인", full_text, height=450)
            except Exception as e:
